...
pub async fn connect(app_state: AppState, update_sender: UpdateSender) -> Result<(), Box<dyn Error>> {
    ...
    handle_incomming_messages(&mut read, sequence_tracker.clone(), app_state, update_sender).await?;
